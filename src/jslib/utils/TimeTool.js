/**
 * Created by zhangbo on 2018/11/24.
 */
...
export function getLunarMonthByOffset(calcLunar, $monthOffset = 0)
{
 ...
 */
export function getOffset($date1, $date2)
{
...
/**
 * 获取指定时间戳距离现在已经过去了多少时间
 * @param $time ：时间戳10/13位 number
 * @param $passTime ：格式化界线/s
 * @param $type ：格式化类型默认"mm"
 * @param $debit ：是否补全
 * @returns {String|Array}
 */
export function getPassTime($time = 0, $passTime = 3600, $type = TYPE_M, $debit = false)
{

	var time = 0, passSeconds = 0;
	if ($time.toString().length > 10)
	{
	...
	passSeconds = getNowStamp() - time;
	if (passSeconds >= $passTime)
	{
		return getFullDate(Number($time), $debit)
	}
	else
	{
		return getCountDown(passSeconds, $type)
	}
}
 ...
 * @returns {Array}
 */
export function getDateByOffsetDay($offset, $debit = false)
{

	var time = __START_TIME;
	var offsetSeconds = $offset * 86400000;
	return getDateArray(time - offsetSeconds, $debit);
}

/**
 * 获取指定月份偏移后的日期
 * @param $offset ：偏移月数 本月：0 上一个月：-1 下一个月：1
 * @returns {Array} [startDate,endDate]
 */
export function getDateByOffsetMonth($offset, $debit = false)
{
	var da = new Date();
	var nowY = da.getFullYear();
	var nowM = da.getMonth();
	...
	var start = new Date(y, m, 1);
	var end = new Date(y, m + 1, 0);
  var startDate = getDateArray(start, $debit);
	var endDate = [];
	if ($offset === 0)
	{
    endDate = getDateArray(__START_TIME,$debit)
	}else{
    endDate = getDateArray(end, $debit);
	}

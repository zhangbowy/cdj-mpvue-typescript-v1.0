...
export function call(...args)
{
	...
	param.header["Cookie"] = wx.getStorageSync("cookie") ? `openid=${wx.getStorageSync("cookie")}` : "";
	if (param.params)
	{
		param.params = util.__merge(param.params, _globalParams);
	}
	else
	{
		param.params = util.__merge({}, _globalParams);
	}
	...
	function onResult(d, resolve, reject)
	{
		...
		function doFormatResult()
		{
			if (mime == "json")
			{
				if (typeof dObj == "string")
				{
					...
					{
						console.log("[ERROR]服务端返回的数据不是有效的json串!");
					}
				}
		...
		function doNetCallBack()
		{
			...
			if (util.getType(dObj[__data]) != "Object")
			{
				var tempObj = {};
				tempObj[__data] = dObj[__data];
				dObj[__data] = tempObj;
			}
	...
	function onError(d, reject)
	{
		if (util.onMode("debug"))
		{
			console.log("[ERROR]" + param.url.replace(_serverPath, ''), "status:", d.status, d);
		}
	...
	if (!_reqHash[reqKey] || (_reqHash[reqKey] && new Date().getTime() - _reqHash[reqKey].time >= (_httpInfo.repeatReqTime || 2000)))
	{
		...
		{
			if (util.onMode("devData") || args[3]==='mock')
			{ //假数据
				var res = util.__merge({}, mockData[url]);
				onResult(res, resolve, reject);
				return
			}

			if(param.params && g.configs.param.apikey)
      		{
       		    param.params.apikey = g.configs.param.apikey;//apiKey
      		}
			wx.request({
				url: param.url,
...
export function init($serverObj)
{
	...
	_httpInfo = util.__merge(_httpInfo, $serverObj.http, true);
	...
	if (util.onMode("debug"))
	{
		console.log("[GNet:Init]", "SERVER_PATH:\"" + _serverPath + "\"");
	}
}

function setGlobalParams($params, $override)
{
	if ($override)
	{
	...
	{
		_globalParams = util.__merge(_globalParams, $params);
	}
}

function addHook($hookObj)
{
